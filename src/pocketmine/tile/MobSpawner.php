...
class MobSpawner extends Spawnable
{
	...
	public function onUpdate() : bool
	{
		if ($this->entityId !== -1) {
			if ($this->delay++ >= rand($this->minSpawnDelay, $this->maxSpawnDelay)) {
				...
				if ($canSpawnMob && $nearEntityCount <= $this->maxNearbyEntities) {
	...
	{
		return $this->level->getBlock($pos) instanceof Air && $this->level->getBlock($pos->up()) instanceof Air && $this->level->getBlock($pos->down())->isSolid();
	}
}
