...
class Item implements ItemIds, JsonSerializable
{
	 ...
	 * @return Item[]|Item
	 */
	public static function fromString(string $str, bool $multiple = false) : array|Item
	{
	...
	{
		return $this->nbt !== null && $this->nbt->getCount() > 0;
	}

	public function checkCompoundTag() : void
	{
	...
	{
		$display = $this->getNamedTagEntry(self::TAG_DISPLAY);
		if ($display instanceof CompoundTag && ($lore = $display->getListTag(self::TAG_DISPLAY_LORE)) !== null) {
	...
	final public function equals(Item $item, bool $checkDamage = true, bool $checkCompound = true) : bool
	{
		return $this->id === $item->getId() &&
			(!$checkDamage || $this->getDamage() === $item->getDamage()) &&
