...
class Human extends Creature implements ProjectileSource, InventoryHolder
{
	...
	protected static function deserializeSkinNBT(CompoundTag $skinTag) : Skin
	{
		$skin = new Skin(
			...
			$skinTag->hasTag(self::TAG_SKIN_DATA, StringTag::class) ? $skinTag->getString(self::TAG_SKIN_DATA) : $skinTag->getByteArray(self::TAG_SKIN_DATA), //old data (this used to be saved as a StringTag in older versions of PM)
	...
	public function onPickupXp(int $xpValue) : void
	{
		...
		if (($item = $this->inventory->getItemInHand()) instanceof Durable && $item->hasEnchantment(Enchantment::MENDING)) {
			$equipment[$mainHandIndex] = $item;
		}
		if (($item = $this->offHandInventory->getItemInOffHand()) instanceof Durable && $item->hasEnchantment(Enchantment::MENDING)) {
			$equipment[$offHandIndex] = $item;
		}

		foreach ($this->armorInventory->getContents() as $k => $armorItem) {
			if ($armorItem instanceof Durable && $armorItem->hasEnchantment(Enchantment::MENDING)) {
	...
	protected function doFoodTick(int $tickDiff = 1) : void
	{
		if ($this->isAlive()) {
			...
			if ($difficulty === Level::DIFFICULTY_PEACEFUL && $this->foodTickTimer % 10 === 0) {
	...
	public function saveNBT() : void
	{
		...
		if ($this->offHandInventory !== null) {
			$this->namedtag->setTag($this->offHandInventory->getItemInOffHand()->nbtSerialize(0, "OffHand"));
		}
		...
		$this->namedtag->setTag(new CompoundTag(self::TAG_SKIN, [
			new StringTag(self::TAG_SKIN_NAME, $this->skin->getSkinId()),
			new ByteArrayTag(self::TAG_SKIN_DATA, $this->skin->getSkinData()),
			new ByteArrayTag(self::TAG_SKIN_CAPE_DATA, $this->skin->getCapeData()),
			new StringTag(self::TAG_SKIN_GEOMETRY_NAME, $this->skin->getGeometryName()),
			new ByteArrayTag(self::TAG_SKIN_GEOMETRY_DATA, $this->skin->getGeometryData()),
		]));
	}
