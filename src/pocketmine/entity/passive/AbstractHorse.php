...
abstract class AbstractHorse extends Tamable
{
	...
	protected function initEntity() : void
	{
		...
		$this->propertyManager->setByte(self::DATA_CONTAINER_TYPE, WindowTypes::HORSE);
	...
	public function onUpdate(int $currentTick) : bool
	{
		...
		if ($this->rearingCounter > 0 && ++$this->rearingCounter > 20) {
	...
	public function moveWithHeading(float $strafe, float $forward)
	{
		...
		if ($riddenByEntity instanceof Living && $this->isSaddled()) {
			...
			if ($this->onGround && $this->jumpPower === 0 && $this->isRearing() && !$this->isJumpRearing) {
				$strafe = 0;
				$forward = 0;
			}

			if ($this->jumpPower > 0 && !$this->isHorseJumping() && $this->onGround) {
	...
	{
		return parent::canBePushed() && $this->getRiddenByEntity() === null;
	}
}
