...
abstract class Living extends Entity implements Damageable
{
	...
	protected ?CompoundTag $leashNbt = null;
	...
	protected function initEntity() : void
	{
		...
		if ($this->isLeashed() && $this->namedtag->hasTag("Leash", CompoundTag::class)) {
			...
			if ($effect->isVisible() && $effect->getType()->hasBubbles()) {
				...
				if ($packedEffectsCount < 8) {
					$effectsData = ($effectsData << 7) |
						(($effect->getId() & 0x3f) << 1) | //Why not use 7 bits instead of only 6? mojang...
	...
	public function applyDamageModifiers(EntityDamageEvent $source) : void
	{
		...
		if ($this->hasEffect(Effect::DAMAGE_RESISTANCE) && $cause !== EntityDamageEvent::CAUSE_VOID && $cause !== EntityDamageEvent::CAUSE_SUICIDE) {
	...
	public function attack(EntityDamageEvent $source) : void
	{
		if ($this->noDamageTicks > 0) {
			...
			if ($lastCause !== null && $lastCause->getBaseDamage() >= $source->getBaseDamage()) {
				$source->setCancelled();
			}
		}
	...
	public function entityBaseTick(int $tickDiff = 1) : bool
	{
		...
		if ($revengeTarget = $this->getRevengeTarget()) {
			if (!$revengeTarget->isAlive() || ($this->ticksLived - $this->revengeTimer) > 100) {
	...
	protected function doAirSupplyTick(int $tickDiff) : bool
	{
		...
		if (!$this->canBreathe()) {
			...
			if (($respirationLevel = $this->armorInventory->getHelmet()->getEnchantmentLevel(Enchantment::RESPIRATION)) <= 0 ||
	...
	{
		return $this->hasEffect(Effect::WATER_BREATHING) || $this->hasEffect(Effect::CONDUIT_POWER) || !$this->isUnderwater();
	}
	...
	public function recreateLeash() : void
	{
		if ($this->isLeashed() && $this->leashNbt !== null) {
			...
			} elseif ($this->leashNbt->hasTag("X", IntTag::class) && $this->leashNbt->hasTag("Y", IntTag::class) && $this->leashNbt->hasTag("Z", IntTag::class)) {
